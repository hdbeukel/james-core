...
/**
 * Abstract neighbourhood search that uses <i>multiple</i> neighbourhoods to modify the current solution.
 * This includes variable neighbourhood descent and variable neighbourhood search.
 *
...
public abstract class MultiNeighbourhoodSearch<SolutionType extends Solution> extends NeighbourhoodSearch<SolutionType> {
    ...
    // neighbourhoods (unmodifiable view)
     ...
     */
    public MultiNeighbourhoodSearch(Problem<SolutionType> problem,
    ...
    public MultiNeighbourhoodSearch(String name, Problem<SolutionType> problem,
        ...
        if(neighs == null){
            throw new NullPointerException("Error while creating multi neighbourhood search: "
                                            + "neighbourhood list can not be null.");
        }
        // check that neighs does not contain any null elements
        for(Neighbourhood<?> n : neighs){
            if(n == null){
                throw new NullPointerException("Error while creating multi neighbourhood search: "
                                                + "neighbourhood list can not contain any null elements.");
            }
        }
        // check neighs not empty
        if(neighs.isEmpty()){
            throw new IllegalArgumentException("Error while creating multi neighbourhood search: "
                                                + "neighbourhood list can not be empty.");
        }
        // store neighbourhoods
        this.neighs = Collections.unmodifiableList(neighs);
    }
    
    /**
     * Get list of applied neighbourhoods (unmodifiable).
     * 
     * @return neighbourhood list (unmodifiable)
     */
